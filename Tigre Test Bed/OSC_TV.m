function [ fres ] = OS_ASD_POCS (app, proj,geo,angles,maxiter,varargin)
...
%% parse inputs
[beta,beta_red,ng,verbose,alpha,alpha_red,rmax,epsilon,initialBlockSize,finalBlockSize,blockSizeReductionPower,OrderStrategy,nonneg]=parse_inputs(proj,geo,angles,varargin);
...
%         f=f+beta* bsxfun(@times,...
%             ...%1./sum(V(:,:,orig_index{jj}),3),...            
%             Atb((proj(:,:,orig_index{jj}) - Ax(f,geo,alphablocks{jj})), geo, alphablocks{jj})...%Atb(W(:,:,orig_index{jj}) .* (proj(:,:,orig_index{jj}) - Ax(f,geo,alphablocks{jj})), geo, alphablocks{jj})...
%             );
            
        eProj = Ax(f,geo,alphablocks{jj});
        
        if iter == 1
           f = f + ( ...
               Atb(exp(-eProj) - exp(-proj(:,:,orig_index{jj})), geo, alphablocks{jj}) ./...
               (Atb(ones(size(eProj),'single'), geo, alphablocks{jj})));
        else
           f = f + (f .*  ...
               Atb(exp(-eProj) - exp(-proj(:,:,orig_index{jj})), geo, alphablocks{jj}) ./...
               (Atb(ones(size(eProj),'single'), geo, alphablocks{jj})));
...
%     c=dot(dg_vec(:),dp_vec(:))/(norm(dg_vec(:),2)*norm(dp_vec(:),2));
    if iter>=maxiter%(c<-0.99 && dd<=epsilon) || beta<0.005|| iter>=maxiter
        if verbose
...
function [beta,beta_red,ng,verbose,alpha,alpha_red,rmax,epsilon,initialBlockSize,finalBlockSize,blockSizeReductionPower,OrderStrategy,nonneg]=parse_inputs(proj,geo,angles,argin)

opts=     {'lambda','lambda_red','tviter','verbose','alpha','alpha_red','ratio','maxl2err','initialblocksize','finalblocksize','blocksizereductionpower','orderstrategy','nonneg'};
    ...
    switch opt
        ...
        case 'blocksizereductionpower'
            if default
                ...
                if length(val)>1 || ~isnumeric( val)
                    error('OS_ASD_POCS:InvalidInput','Invalid Block Size Reduction Power')
                end
