function [ fres ] = OSC_TV2 (app, projData, projRef, geo,angles,maxiter,varargin)
...
%% parse inputs
[beta,beta_red,ng,verbose,alpha,alpha_red,rmax,epsilon,initialBlockSize,finalBlockSize,blockSizeReductionPower,OrderStrategy,nonneg]=parse_inputs(projData,geo,angles,varargin);
...
%         if iter == 1
%            f = f + ( ...
%                Atb(projRef(:,:,orig_index{jj}).*exp(-eProj) - projData(:,:,orig_index{jj}), geo, alphablocks{jj}) ./...
%                (Atb(projRef(:,:,orig_index{jj}), geo, alphablocks{jj})));
%         else
           f = f + (f .*  ...
                Atb(projRef(:,:,orig_index{jj}).*exp(-eProj) - projData(:,:,orig_index{jj}), geo, alphablocks{jj}) ./...
